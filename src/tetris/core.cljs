...
(def COLS 10)
(def ROWS 20)
; TODO takes only height into consideration
...
(defn fall-interval [level]
  (let [dec-per-level (quot MAX-FALL-INTERVAL MAX-LEVEL)]
    (max TICK-INTERVAL 
       (- MAX-FALL-INTERVAL (* dec-per-level (dec level))))))

; TODO base this on piece width
...
(defn make-falling-piece-fall! [game] 
  (move-falling-piece! game :down))
...
(defn falling-piece-to-landed-piece [game]
  (swap! game (fn [g]
            ...
            (let [landed? (not (make-falling-piece-fall! game))]
              (when landed?
                (if-not (entered-board? (:falling-piece @game))
                  (game-over! game)
                  (do
                    (falling-piece-to-landed-piece game)
